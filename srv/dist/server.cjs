...
var DatabasePostgres = class {
  async list(search) {
    let videos;
    if (search) {
      videos = await sql`select * from post where title ilike ${"%" + search + "%"}`;
    } else {
      videos = await sql`select * from post`;
    }
    console.log("allVideos");
    return videos;
  }
  async create(post) {
    const videoId = (0, import_crypto.randomUUID)();
...
var apiController = () => {
  const createVideo = async (request, reply) => {
  ...
  const getVideos = async (request, reply) => {
    const search = request.query.search;
    const videos = await database.list(search);
    return reply.send(videos);
  };
  const editVideo = async (request, reply) => {
  ...
  const deleteVideo = async (request, reply) => {
    const videoId = request.params.id;
...
var controller = apiController_default();
var routes = async (fastify3) => {
...
// src/server.ts
var server2 = (0, import_fastify2.default)();
server2.register(routes_default);
var port = 3e3;
server2.listen({ host: "0.0.0.0", port }, (err, address) => {
  if (err) {
    console.error(err);
    process.exit(1);
  }
